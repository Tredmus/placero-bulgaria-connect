...
/* ---------------- helpers to normalize + build country mask ---------------- */

type Ring = [number, number][];
const isCoord = (x: any): x is [number, number] => Array.isArray(x) && x.length === 2 && isFinite(x[0]) && isFinite(x[1]);
const isRing = (r: any): r is Ring => Array.isArray(r) && r.length >= 4 && r.every(isCoord);
...
function normalizeFC(raw: any) {
  if (!raw || raw.type !== 'FeatureCollection') return { type: 'FeatureCollection', features: [] };
  const features = (raw.features || [])
    ...
    .map((f: any) => {
      const g = f.geometry;
      if (g.type === 'Polygon') {
        g.coordinates = (g.coordinates || []).map((ring: any) => (isRing(ring) ? closeRing(ring) : ring)).filter(isRing);
      ...
      let cleaned: any = cleanCoords(f, { mutate: false });
      try { cleaned = rewind(cleaned, { reverse: false, mutate: false }); } catch {}
      return cleaned;
    });
  return { type: 'FeatureCollection', features };
}

function unionAll(features: any[]) {
  ...
  for (let i = 1; i < features.length; i++) {
    try {
      ...
      // if union fails on a pair, skip that feature rather than crash
      continue;
    }
  }
  // Ensure orientation again after union
  try { acc = rewind(cleanCoords(acc, { mutate: false }) as any, { reverse: false, mutate: false }); } catch {}
  return acc;
}

function extractCountryOuterRings(geom: any): Ring[] {
  const out: Ring[] = [];
  if (!geom) return out;
  if (geom.type === 'Polygon') {
    if (isRing(geom.coordinates?.[0])) out.push(geom.coordinates[0]);
  } else if (geom.type === 'MultiPolygon') {
    for (const poly of geom.coordinates || []) {
      if (isRing(poly?.[0])) out.push(poly[0]);
    }
  }
  return out;
}
...
export default function InteractiveMap() {
  ...
  const [countryMask, setCountryMask] = useState<any>(null); // single mask from dissolved country
  ...
  // normalize + dissolve into one country geometry, then build world mask
  useEffect(() => {
    if (!rawProvincesGeo) return;
    const normalized = normalizeFC(rawProvincesGeo);
    setProvincesGeo(normalized);

    const dissolved = unionAll(normalized.features);
    if (!dissolved) return;

    const worldRing: Ring = [[-180, -85], [180, -85], [180, 85], [-180, 85], [-180, -85]];
    const outers = extractCountryOuterRings(dissolved.geometry);
    const mask = {
      type: 'Feature',
      properties: {},
  ...
  useEffect(() => {
    if (!mapEl.current || !token || !provincesGeo || !countryMask) return;
    ...
    map.current.on('load', () => {
      // dark world outside Bulgaria
      map.current!.addSource('world-mask', { type: 'geojson', data: countryMask });
      ...
      // Provinces (normal, still separate for selection)
  ...
  }, [token, provincesGeo, countryMask, handleProvinceSelect]);
